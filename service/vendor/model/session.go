...
func init() {
	...
	if !agendaDB.HasTable(s) {
		err := agendaDB.CreateTable(s).Error
		util.PanicIf(err)
		log.Infof("\n ...... CreateTable %T. \n", s)
	}
}
...
func (*SessionInfoAtomicService) FindAll() ([]entity.SessionInfo, error) {
	...
	err := agendaDB.Find(&rows).Error // CHECK: should check .Error ?
	return rows, err
}
...
func (*SessionInfoAtomicService) FindByToken(token entity.Token) (entity.SessionInfo, error) {
	...
	err := agendaDB.First(&sInfo, entity.SessionInfo{Token: token}).Error
	return sInfo, err
}
